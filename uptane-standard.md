...
## Data tables & sample data structures

### A. Data Table {#data_table}

>All data structure samples are represented in json format. Implementators MAY choose a different notation for data interchange.

#### A.1 Type of key(s) required for each entity to perform its task

Entity | Keys required to perform its function|
-------|-----|
TimeServer | Timeserver keys |
Director | Root keys, Timeserver keys, Sanpshot keys and Target keys |
Repository | A directory structure or location on storage device where keys, metadata or images are stored|
Primary ECU | Primary keys, Timeserver public key|
Secondary ECU | ECU keys, Timserver public key, Director public key|
>table a.1
>

#### A.2 Information storing entities

Entity | Description|
-------|------------|
Inventory Database | Director referes to inventory database to provide latest update information related to VIN, and ECU serial number ( or an hardware id) |
Image Repository| It will host images or update bundles and can be part of Content Delivery Network (CDN)|
>table a.2
>

### B. Important data structures that become part of wire message

#### B.1 signatures data structure {#signatures}

signatures MUST have atleast one signature. It MAY have more than one signature.

 field | type |
-------|------|
keyid  | String of 64 hexadecimal digits |
sig    | String of hexadecimal digits|
method | "ed25519" or "rsa" (currently supported)|
>table b.1.1
...
#### B.2 hashes data structure {#hashes}

Hashes are digital signatures generated over some data using private key.

 field | type |
-------|------|
sha256 | String of 64 hexadecimal digits|
sha512 | String of 128 hexadecimal digits|
>table b.2.1
...
```
#### B.3 fileinfo data structure (#fileinfo)
...
#### B.4 installed_image data structure {#installed_image}
...
#### B.5 signed data structure for ecu manifest (#signecuobject)

field | type |
---------------|------|
attacks_detected| int|
previous_timeserver_time | datetime |
ecu_serial | string |
installed_image| {{#installed_image}}|
timeserver_time| datetime|
>table b.5.1
...
#### B.6 ECU manifest data structure (#ecumanifest)

field | type |
------|------|
signatures|{{#signatures}}|
signed |{{#signedecuobject}}|
>table b.6.1
...
field| type|
-----|-----|
ecu_version_manifests| {{#ecumanifest}} |
...
### C. Update cycle communication profiles

 A message format is divided into two parts:

      1. signatures
      2. signed

> signatures is an array. It MUST have atleast one array element with the following information:
>
>      a. keyid  : unique id for a key that was used to sign the data in signed section
>      b. sig    : signature generated for the data in signed section
>      c. method : ed25519|rsa
>
> signed : Elements of this part MAY differ depending upon the type to message
> 
>Note:
> 1. signature section is part of all messages with the same data strucutre. Hence it will only be mentioned in json samples from here on.
> 2. message entities that consists of sub entities will be explained below the message table.


#### C.1 Communication with Timeserver (if implemented)

| Entity | Keys|
|--------|-----|
|Timeserver| Timeserver keys|
|Primary| Primary keys,Timeserver public key|
|Secondary| ECU keys, Timeserver public key, and Director's public key|
>table c.1.1


Timeserver Request MUST have:

*ECU serial number
*tokens/nonces from each ECU

sample request:
>Note: This sample MAY contain extra fields.
...
Timeserver response MUST have:

* signatures ( 1 or many)
* signed over current timestamp from timeserver, and nonces that were sent in the request.
>Note:Timezone implementation is not in scope of this document
...
Field|Description|
-----|-----------|
nounces |random Interger|
time    |String for datatime|
>table c.1.2


sample response from Timeserver:
...
Field|Description|
-----|-----------|
(underscore)type | role name |
roles|contains keyid & threshold signatures required|
keys |detailed information for each keyid used to sign for different roles. Private key MUST not mentioned |
expires| datetime when this information would be considered invalid|
version| integer value|
consistent_snapshot| boolean |
compression_algorithms| array of strings|
>table d.1.1.1

roles: identified individually by role name
role name has:

Field|Description|
-----|-----------|
threshold| integer for number os signatures must be verified for this role. The value MUST never be less than 1|
keyids| array of keyids greater than or equal to the threshold|
>table d.1.1.2

keys: contains data for each key used for signing the data
Every keyid has the following details

Field|Description|
-----|-----------|
keytype|String containing either ed25519 or rsa|
key_hash_algorithms| array of string of hashing algorithm(s) used|
key_val| data structure containing element 'public' whose value is public key |
>table: d.1.1.3
...
```
##### D.1.2 Snapshot Metadata

signed section

Field|Description|
-----|-----------|
(underscore)type| role_name|
expires| datetime when the information expires|
version| version of this file|
meta| contains data for target files version numberand filename. It may list root metadata's information|
>table: d.1.2.1
...
expires| datetime when the information expires|
version| version of this file|
delegations| contains information on delegations|
targets| One or more ECUs with information about ECU serial number,version, fieldname, and location of new image or update in local repository |
>table d.1.3.1
